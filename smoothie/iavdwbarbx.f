...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       subroutine NEB_DWBA_Zero_Spin_rbx()
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        use gauss
        use spharm
        use input, only:prior,icf,ZR,VF
        use zerorangesource
        implicit none
...
C          call OMP_SET_NUM_THREADS(1)
 ...
 94      format("Using finite range DWBA in IVA-post form")
          ...
          if(VF) call init_Pi_ext(counter)  
          if(ZR .or. VF) call D0factor() 
          if(.not. ZR)call initial_func_pot_in_rbx()
          write(*,96)
96        format( "calculating cross section alpha_xA dependence ",/)
          ...
          do alpha_xA=1, out2b%nchmax
              ...
              call sigma_J_pi_rbx(alpha_xA,xsec_alpha_xA(alpha_xA),kplb,dsdedw,xsecla_lx,xseclb_lx)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine sigma_J_pi_rbx(alpha_xA,xsec_sum,kplb,dsdedw,xsec_la,xsec_lb)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      use channels
      use mesh
      use zerorangesource
...
!$OMP DO schedule(dynamic)
       do nth=0,nthmax
          ...
          call dsdedwFR_rbx(dsdedw(nth),kplb(:,:,nth),abarmax,alpha_xA)
...
c-----------------------------------------------------------------------
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine dsdedwFR_rbx(xsec,kplb,abarmax,alpha_xA)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      use channels
      use precision
      implicit none
...
c      do alpha_xA=1,out2b%nchmax
        ...
        do mxA=-lx,lx
          do mbx=-lbx,lbx
            call psiintegration_rbx(alpha_xA,abarmax,mbx,mxA,kplb,xsec_alpha_xA)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        use systems
        use constants
        use input,only:prior,printf,ut,VF,ZR
        use zerorangesource
        implicit none
        integer :: alphabar
        ...
        complex*16, dimension(1:nr)  :: R_ext_val
        ...
        if(.not. ZR)  then 
!       compute finite range source term
...
!       compute the Zero range source term 
        call rho_func_zero(alphabar,rho)    
        
        end if 


        if(VF) call R_ext(alphabar, R_ext_val)  
        ...
        do irx=1,nr
          ...
          if(VF)  R_func(irx)=R_func(irx) + R_ext_val(irx)
          if((.not. ZR) .and. prior .and. (.not. UT)) R_func(irx)=R_func(irx)+R_funcNO(irx)* 2.**4 * pi**2/ka/kb
