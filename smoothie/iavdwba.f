      ...
      use fuspot
      implicit none
       complex*16,dimension(:,:,:),allocatable :: wf_b_store,UbB_store, UxA_store
       ...
       integer :: NUM_openmp ! threshold number


      contains

ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       subroutine NEB_DWBA_with_Spin()
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        integer :: nth,ix
        real*8,dimension(0:nthmax,0:lmax,0:lmax) :: kplb
        real*8,dimension(0:lmax,0:lmax) :: kpl
...
!$OMP PARALLEL
        NUM_openmp= OMP_GET_NUM_THREADS()
!$OMP END PARALLEL
...
!$OMP PARALLEL default(shared) private(nth,theta,cth,kpl)
!$OMP DO schedule(dynamic)
      do nth=0,nthmax
         theta=thmin+nth*thinc
         cth=cos(theta*pi/180.)
         call PLM(cth,lmax,lmax,lmax+1,kpl(0:lmax,0:lmax))
         kplb(nth,0:lmax,0:lmax)= kpl(0:lmax,0:lmax)
      end do
!$OMP END DO
!$OMP end PARALLEL
        ...
        do counter=1,neb
          ...
          call Pauli_state(counter)
          if (ecmx<0) call Pauli_state(counter)
...
C          call Green_func_nonlocalpot(ecmx,0)
...
96        format( "calculating cross section alpha_xA dependence ",/)
          ...
          do alpha_xA=1, out2b%nchmax
              ...
              call sigma_J_pi_DWBA (alpha_xA,xsec_alpha_xA(alpha_xA),kplb,dsdedw,xsecla_lx,xseclb_lx)
...
c-----------------------------------------------------------------------
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine dsdedwFR_DWBA(xsec,kplb,abarmax,alpha_xA)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      use channels
      use mesh
      use precision
      implicit none
      real*8,dimension(0:nthmax) :: xsec,xsec_alpha_xA
      integer :: mxA_2,mbx_2,alpha_xA,abarmax,jxA_2
      integer :: jbx_2,jA_2,jb_2,mjA_2,mjb_2
      real*8 :: jxA
      real*8,dimension(0:nthmax,0:lmax,0:lmax) :: kplb
...
c      do alpha_xA=1,out2b%nchmax
        jxA=out2b%j(alpha_xA)
        jxA_2=nint(2.*jxA)

        do mxA_2=-jxA_2,jxA_2,2
          do mjA_2=-jA_2,jA_2,2
            do mbx_2=-jbx_2,jbx_2,2
               do mjb_2=-jb_2,jb_2,2
                 call psiintegration_DWBA(alpha_xA,abarmax,mbx_2,mjA_2,mxA_2,mjb_2,kplb,xsec_alpha_xA)
...
c-----------------------------------------------------------------------
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine psiintegration_DWBA(alpha_xA,abarmax,mbx_2,mjA_2,mxA_2,mjb_2,PL,xsec_alpha_xA)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      use channels
      use precision
      use constants
      ...
      use input,only:icf
      implicit none
      real*8,dimension(0:nthmax) :: xsec_alpha_xA
      integer :: alphabar,alpha_xA,abarmax
      integer :: mbx_2,mjA_2,mxA_2,mjb_2
      complex*16,dimension(1:nr,0:nthmax) :: Psi
      complex*16,dimension(1:nr,0:nthmax,1:abarmax) :: Psi_parallel
      integer :: irx
      real*8 :: rx
      ...
      real*8 :: abs_R
      integer :: nth
      real*8,dimension(0:nthmax,0:lmax,0:lmax) :: PL
      N=-mua * mub * kb / 4 / pi**3 / hbarc**4 /ka /(2.*jt+1.)/(2.*jp+1.)
      N=N*10.0_dpreal ! 1fm2=10mb
      Psi=0.0_dpreal
!$OMP PARALLEL default(shared) private(alphabar)
!$OMP DO schedule(dynamic)
      do alphabar=1,abarmax
         CALL psiintegration_parallel(alpha_xA,alphabar,mbx_2,mjA_2,mxA_2,mjb_2,PL,Psi_parallel(:,:,alphabar))
      end do
!$OMP END DO
!$OMP end PARALLEL
      ...
      do nth=0, nthmax
        do irx=1,nr
           rx=rr(irx)
           abs_R= abs(Psi(irx,nth))**2
           xsec_alpha_xA(nth)=xsec_alpha_xA(nth) + rx**2 * FFR4(rx/hcm,Wx,irmatch+1) * abs_R  * rrw(irx)
        end do
      end do
...
c-----------------------------------------------------------------------
      subroutine psiintegration_parallel(alpha_xA,alphabar,mbx_2,mjA_2,mxA_2,mjb_2,PL,Psi)
! this subroutine used to parallel the angular part
! in terms of alphabar index
C-------------------------------------------------------------------------------------------------------
      use channels
      use precision
      use constants
      ...
      integer :: mbx_2,mjA_2,mxA_2,mjb_2
      real*8 :: Jtot,Jbb,JAA
      integer :: Jtot_2,MJ_2,Jbb_2,MJbb_2,mlamb_2,JA_2,JAA_2
      integer :: Jbx_2,jb_2,Jx_2
      complex*16,dimension(1:nr,0:nthmax) :: Psi
      real*8 :: Yla
      real*8,dimension(0:nthmax):: Ylb,YY
      ! real*8 :: N,abs_R
      real*8,dimension(0:nthmax,0:lmax,0:lmax) :: PL
      real*8 :: CGin,CGout
      integer :: nth
      real*8 :: minus
      Jx_2=nint(2.*out2b%j(alpha_xA))
      JA_2=nint(2.*jt)
      Jbx_2=nint(2.*jp)
      jb_2=nint(2.*jb)
      ma=0
      ...
      JAA=in3b%j_spect(alpha_in)
      JAA_2=nint(2.*JAA)
      Jbb=out3b%j_spect(alpha_out)
      Jbb_2=nint(2.*Jbb)
      Jtot=(in3b%j(alpha_in))
      Jtot_2=nint(2.*Jtot)
      YY=0.0_dpreal
      ...
      do mlamb=-lb,lb
        mlamb_2=nint(2.*mlamb)
        ylb(:)=YLMC(lb,mlamb)*PL(:,lb,abs(mlamb))*exp(-iu*abs(mlamb)*pi)
        do MJ_2=-Jtot_2,Jtot_2,2
          do MJbb_2=-Jbb_2,Jbb_2,2
            if(mlamb_2 - mjb_2  /= MJbb_2) cycle
            if((mxA_2+MJbb_2)/=MJ_2) cycle
            if((mbx_2+mjA_2)/=MJ_2) cycle
            CGin=cleb(la*2,ma,JA_2,mjA_2,JAA_2,mjA_2)*cleb(Jbx_2,mbx_2,JAA_2,mjA_2,Jtot_2,MJ_2)
            CGout=cleb(lb*2,mlamb_2,jb_2,-mjb_2,Jbb_2,MJbb_2)*cleb(Jx_2,mxA_2,Jbb_2,MJbb_2,Jtot_2,MJ_2)
            CGout=CGout * minus**((jb_2+mjb_2)*0.5_dpreal)
      ...
      do nth=0,nthmax
         Psi(:,nth)=YY(nth)*Rabar(:,alphabar)
      end do


      end subroutine
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine sigma_J_pi_DWBA(alpha_xA,xsec_sum,kplb,dsdedw,xsec_la,xsec_lb)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      use channels
      use mesh
      implicit none
...
C        chunk_size=int(real(abarmax)/real(NUM_openmp))

c        write(*,*)"abarmax=",abarmax, "NUM_openmp=",NUM_openmp ,"chunk_siz=", chunk_size
...
!$OMP DO schedule(dynamic)
        do alphabar=1,abarmax
          call dsde_DWBA(alphabar,xsec(alphabar) )
        end do
!$OMP END DO
!$OMP end PARALLEL
...
C      chunk_size=int(real(nthmax+1)/real(NUM_openmp))



      call dsdedwFR_DWBA(dsdedw(:),kplb(:,:,:),abarmax,alpha_xA)
      ...
      if (printlx) then
        do ith=0,nthmax
           theta=thmin+ith*thinc
           write(1000+alpha_xA,*)theta,dsdedw(ith)
        end do
        write(1000+alpha_xA,*)"&"
      end if
...
c-----------------------------------------------------------------------
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       subroutine dsde_DWBA(alphabar,xsec)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        call Rfunc_DWBA(alphabar,R_func)
        Rabar(:,alphabar)=R_func
        do irx=1,nr
          rx=rr(irx)
          abs_R= abs(R_func(irx))**2 * (2.0_dpreal*J_out+1.0_dpreal)/4.0_dpreal/pi
          xsec=xsec + rx**2 * FFR4(rx/hcm,Wx,irmatch+1) * abs_R  * rrw(irx)
        end do



      xsec=xsec*N/(2.*jt+1.)/(2.*jp+1.)


       end subroutine
c-----------------------------------------------------------------------
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       subroutine Rfunc_DWBA(alphabar,R_func)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        call G_coeffcient(alphabar,Gabar)
        ...
        do irxp=1,nr
          ...
          do irb=1,nr
            ...
            if(prior) then
               ...
               call lambda_func_prior_DWBA(alphabar,G_alphabar,irb,irxp,lambda,lambdaNO)
              ...
              call lambda_func_DWBA(alphabar,G_alphabar,irb,irxp,lambda)
              rho(irxp)=rho(irxp) + rb * iu**(-lb) * wf_b(irb,alphaspect) * lambda * rrw(irb)
            end if
          end do
...
C           GG=-2.0_dpreal*mux/hbarc/hbarc/kx/rx/rr(10)* Gx%re(min(irx,10),alpha2b) * Gx%ir(max(irx,10),alpha2b)
C      write(111,*) rx, real(GG),imag(GG)

          if(prior .and. (.not. UT)) R_func(irx)=R_func(irx)+R_funcNO(irx)* 2.**4 * pi**2/ka/kb
...
c-----------------------------------------------------------------------
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       subroutine lambda_func_DWBA(alphabar,G_alphabar,irb,irx,lambda)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       subroutine lambda_func_prior_DWBA(alphabar,G_alphabar,irb,irx,lambda,lambdaNO)
C       this subroutine is used to calculate the DELTA function that
C       appears in the notes
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        use pot
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine G_coeffcient(alphabar,Gabar)
c     this subroutine is used to calculate the G coefficients
c     with all particle spins
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      use mesh
      use channels
      use cleb_coefficient
      ...
      integer :: LL, nS,LLmin,LLmax,nSmin,nSmax
      real*8 :: SS,sxA,JxA,Jbb,Jbx,jA,JAA,Jtot
      integer :: lx,lamb,lama
      real*8,dimension(1:nx,1:nr,1:nr) :: Gabar,Y4
      real*8 ::c3u9coef,c6u9coef,u6coef
      real*8  :: NA

      alpha_out=abar%alpha_out(alphabar)
      alpha_in=abar%alpha_in(alphabar)

      lama=in3b%lam(alpha_in)
C      lbx=in3b%l(alpha_in)
C      sbx=in3b%s2b(alpha_in)
      Jbx=in3b%J2b(alpha_in)
      jA=jt
      JAA=in3b%j_spect(alpha_in)
      Jtot=in3b%J(alpha_in)

      lx=out3b%l(alpha_out)
      lamb=out3b%lam(alpha_out)
      sxA=out3b%s2b(alpha_out)
      JxA=out3b%J2b(alpha_out)
!     jb=jb
      Jbb=out3b%j_spect(alpha_out)


      LLmin=max(abs(lx-lamb), abs(lbx-lama))
      LLmax=min(lx+lamb,lbx+lama)


      nSmin=nint(2.*max(abs(sxA-jb), abs(sbx-jA)))
      nSmax=nint(2.*min(sxA+jb, sbx+jA))
     ...
     +      nint(2.*(LL+SS))<nint(2.*Jtot)) cycle
           ...
           u6coef=rac(jA,jx,SS,jb,sxA,sbx)
...
c-----------------------------------------------------------------------
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine Y_Y_couplings(LL,alphabar,Y4)
c     this subroutine is used to calculate Y4 coupling coefficients
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      use channels
      use precision
      use mesh
