...
C
     ...
     X     R20,ASYM,XNEAR,NDROP / 3., 3., .5, 5 /, ACCUR / 1d-30 /
...
C
   20 IF(AXIAL) THEN
C                                                        REAL VERSION
      F = CF1R(real(X),real(ETA),real(ZLL),ACC8,SF ,RK,  ETANE0,LIMIT,ERR,NFP,
...
C
      X = REAL(Z,kreal)
      Y = AIMAG(Z)
      ! Use a much smaller factor than 5 (e.g., 0.1) or remove zeroing completely
      ! AZ= (ABS(X) + ABS(Y)) * ACC * 5
      ! IF(ABS(X) .LT. AZ) X = 0D+0
      ! IF(ABS(Y) .LT. AZ) Y = 0D+0
      TIDY = Z  ! Just return Z without "tidying"
      RETURN
      END FUNCTION
